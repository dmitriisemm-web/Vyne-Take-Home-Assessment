...
test.describe('Check Posts endpoint functionalities', () => {
  test('Verify GET /posts returns all posts', { tag: [TAGS.FUNCTIONAL, TAGS.SMOKE] }, async ({ app }) => {
    const response = await test.step('Request all posts', async () => {
      return app.postsClient.getAll();
    });
  ...
  test('Verify GET /posts/:id returns the correct post shape', { tag: [TAGS.FUNCTIONAL, TAGS.SMOKE] }, async ({ app }) => {
    const response = await test.step('Request a single post', async () => {
      return app.postsClient.getById(VALID_ID);
    });
  ...
  test('Verify POST /posts creates a post with the expected fields', { tag: [TAGS.FUNCTIONAL, TAGS.SMOKE] }, async ({ app }) => {
    const response = await test.step('Create a post', async () => {
      return app.postsClient.create(VALID_POST_PAYLOAD);
    });
  ...
  test('Verify PUT /posts/:id fully replaces the post', { tag: [TAGS.FUNCTIONAL, TAGS.SMOKE] }, async ({ app }) => {
    ...
    const response = await test.step('Replace the post', async () => {
      return app.postsClient.update(VALID_ID, replacement);
    });
  ...
  test('Verify DELETE /posts/:id succeeds', { tag: [TAGS.FUNCTIONAL, TAGS.SMOKE] }, async ({ app }) => {
    const response = await test.step('Delete the post', async () => {
      return app.postsClient.delete(VALID_ID);
    });
  ...
  test('Verify PATCH /posts/:id partially updates the post, preserving other fields', { tag: [TAGS.FUNCTIONAL, TAGS.REGRESSION] }, async ({ app }) => {
    const original = await test.step('Read the original post', async () => {
      const response = await app.postsClient.getById(VALID_ID);
      return getJson<Post>(response);
    });

    const response = await test.step('Patch only the title', async () => {
      return app.postsClient.patch(VALID_ID, { title: 'patched title' });
    });
  ...
  test('Verify GET /posts?userId= filters posts by user', { tag: [TAGS.FUNCTIONAL, TAGS.REGRESSION] }, async ({ app }) => {
    const response = await test.step('Request posts filtered by userId', async () => {
      return app.postsClient.getAll({ userId: VALID_ID });
    });
  ...
  test('Verify GET /posts?userId= with no matches returns an empty array', { tag: [TAGS.FUNCTIONAL, TAGS.REGRESSION] }, async ({ app }) => {
    const response = await test.step('Request posts for a userId that owns none', async () => {
      return app.postsClient.getAll({ userId: 9999 });
    });
  ...
  test('Verify pagination with _page and _limit returns the correct slice', { tag: [TAGS.FUNCTIONAL, TAGS.REGRESSION] }, async ({ app }) => {
    const response = await test.step('Request page 2 with a limit of 10', async () => {
      return app.postsClient.getAll({ _page: 2, _limit: 10 });
    });
  ...
  test('Verify sorting with _sort and _order returns posts in the requested order', { tag: [TAGS.FUNCTIONAL, TAGS.REGRESSION] }, async ({ app }) => {
    const response = await test.step('Request posts sorted by id descending', async () => {
      return app.postsClient.getAll({ _sort: 'id', _order: 'desc', _limit: 3 });
    });
  ...
  test('Verify GET /posts responses include a JSON content-type header', { tag: [TAGS.FUNCTIONAL, TAGS.REGRESSION] }, async ({ app }) => {
    const response = await test.step('Request all posts', async () => {
      return app.postsClient.getAll();
    });
