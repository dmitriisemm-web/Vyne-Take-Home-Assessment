...
test.describe('Check Inventory page functionalities', () => {
  ...
  test('Verify adding a product updates the cart badge and button label', { tag: [TAGS.FUNCTIONAL, TAGS.REGRESSION] }, async ({ saucedemoApp }) => {
    ...
    await test.step('Verify the cart badge and button label update', async () => {
      expect(await saucedemoApp.inventoryPage.header.getCartCount()).toBe(1);
      await expect(saucedemoApp.inventoryPage.removeButton(PRODUCT_NAMES.BACKPACK)).toBeVisible();
    });
  });

  test('Verify removing a product updates the cart badge and button label', { tag: [TAGS.FUNCTIONAL, TAGS.REGRESSION] }, async ({ saucedemoApp }) => {
    await test.step('Add then remove a product from the cart', async () => {
      await saucedemoApp.inventoryPage.addToCart(PRODUCT_NAMES.BACKPACK);
      await saucedemoApp.inventoryPage.removeFromCart(PRODUCT_NAMES.BACKPACK);
    });
    ...
    await test.step('Verify the cart badge and button label reset', async () => {
      ...
      await expect(saucedemoApp.inventoryPage.addToCartButton(PRODUCT_NAMES.BACKPACK)).toBeVisible();
    });
  });
  ...
  test(
    ...
    async ({ saucedemoApp }) => {
      await test.step('Add a product to the cart from the product detail page', async () => {
        await saucedemoApp.inventoryPage.openProduct(PRODUCT_NAMES.BIKE_LIGHT);
      ...
      await test.step('Verify the product detail page reflects the cart state', async () => {
        await expect(saucedemoApp.productDetailPage.removeButton).toBeVisible();
      });

      await test.step('Verify the inventory page reflects the same cart state', async () => {
        await saucedemoApp.productDetailPage.backToProducts();
        await expect(saucedemoApp.inventoryPage.removeButton(PRODUCT_NAMES.BIKE_LIGHT)).toBeVisible();
      });
    }
  );

  test('Verify sorting by name A to Z orders products alphabetically', { tag: [TAGS.FUNCTIONAL, TAGS.REGRESSION] }, async ({ saucedemoApp }) => {
    await test.step('Sort by name Z to A, then back to A to Z', async () => {
      await saucedemoApp.inventoryPage.sortBy(SORT_OPTIONS.NAME_Z_TO_A);
      await saucedemoApp.inventoryPage.sortBy(SORT_OPTIONS.NAME_A_TO_Z);
    });

    await test.step('Verify products are ordered alphabetically', async () => {
      await expect
        .poll(() => saucedemoApp.inventoryPage.getProductNamesInOrder())
        .toEqual([
  ...
  test('Verify sorting by name Z to A orders products in reverse', { tag: [TAGS.FUNCTIONAL, TAGS.REGRESSION] }, async ({ saucedemoApp }) => {
  ...
  test('Verify sorting by price low to high orders products ascending', { tag: [TAGS.FUNCTIONAL, TAGS.REGRESSION] }, async ({ saucedemoApp }) => {
    await test.step('Sort by price low to high', async () => {
      await saucedemoApp.inventoryPage.sortBy(SORT_OPTIONS.PRICE_LOW_TO_HIGH);
    });
  ...
  test('Verify sorting by price high to low orders products descending', { tag: [TAGS.FUNCTIONAL, TAGS.REGRESSION] }, async ({ saucedemoApp }) => {
    await test.step('Sort by price high to low', async () => {
      await saucedemoApp.inventoryPage.sortBy(SORT_OPTIONS.PRICE_HIGH_TO_LOW);
    });
  ...
  test('Verify the cart badge stays accurate across multiple products', { tag: [TAGS.FUNCTIONAL, TAGS.REGRESSION] }, async ({ saucedemoApp }) => {
    await test.step('Add two products to the cart', async () => {
      await saucedemoApp.inventoryPage.addToCart(PRODUCT_NAMES.BACKPACK);
      await saucedemoApp.inventoryPage.addToCart(PRODUCT_NAMES.BIKE_LIGHT);
    });

    await test.step('Verify the cart badge counts both products', async () => {
      expect(await saucedemoApp.inventoryPage.header.getCartCount()).toBe(2);
      await expect(saucedemoApp.inventoryPage.removeButton(PRODUCT_NAMES.BACKPACK)).toBeVisible();
      await expect(saucedemoApp.inventoryPage.removeButton(PRODUCT_NAMES.BIKE_LIGHT)).toBeVisible();
    });
  ...
  test('Verify cart persists across a page reload', { tag: [TAGS.FUNCTIONAL, TAGS.REGRESSION] }, async ({ saucedemoApp, page }) => {
    await test.step('Add a product to the cart', async () => {
      await saucedemoApp.inventoryPage.addToCart(PRODUCT_NAMES.BACKPACK);
      expect(await saucedemoApp.inventoryPage.header.getCartCount()).toBe(1);
    });
